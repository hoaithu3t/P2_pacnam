                ...
                for action in state.getLegalActions(ghost_index):
                    if ghost_index == gameState.getNumAgents() - 1:
                        value = min(
                            value,
                            findMax(
                                state.generateSuccessor(ghost_index, action),
                                depth + 1))
                    else:
                        value = min(
                            value,
        ...
        def findMax(state, depth):
            if isEnd(state, depth):
                ...
                for action in state.getLegalActions(0):
                    value = max(
                        value,
                        findMin(state.generateSuccessor(0, action), depth, 1))
                return value

        actionsWithScore = [(action,
                             findMin(gameState.generateSuccessor(0, action), 0,
                                     1))
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
                ...
                for action in state.getLegalActions(ghost_index):
                    if ghost_index == state.getNumAgents() - 1:
                        ...
                        value = min(
                            ...
                            findMin(
                                state.generateSuccessor(ghost_index, action),
                                depth, ghost_index + 1, alpha, beta))
                    if value < alpha:
                        return value
                    beta = min(beta, value)
                return value
        ...
        optimalAction = None
        for action in gameState.getLegalActions(0):
            nextValue = findMin(gameState.generateSuccessor(0, action), 0, 1,
                                alpha, beta)
        ...
        def findMax(state, depth):
            if isEnd(state, depth):
                ...
                for action in state.getLegalActions(0):
                    value = max(
                        value,
                        findAvg(state.generateSuccessor(0, action), depth, 1))
                return value
...
def betterEvaluationFunction(currentGameState):
    ...
    foodsSortedByDistance = sorted(
        foods, key=lambda food: util.manhattanDistance(food, newPos))
    ghostsSortedByDistance = sorted(
        ghosts, key=lambda ghost: util.manhattanDistance(ghost, newPos))
